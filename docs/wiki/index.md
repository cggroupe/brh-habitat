...
| [agence-lead-economy.md](agence-lead-economy.md) | ⭐ **Phase 16.1 Steps A-C (06/05)** — Modèle économique unifié apporteur d'affaire · 4 sources leads (tier + 3 bonus) · RPC `brh_grant_lead_claim` réécrit · Cascade MLM 5 niveaux (145 € max + 12 leads/charte) · UI breakdown + arbre |
...
| [employe-portal-status.md](employe-portal-status.md) | ⭐ **Phase Employé V1+V2.1→V2.5 (09/05)** — Cockpit gamifié pour commerciaux BRH (Pierre Collard). 7 tables, 4 triggers, 5 fonctions SECURITY DEFINER, 1 EF. Score d'activité 4 niveaux (Standard 5 leads → Master ∞), templates emails recrutement (artisan/agence/architecte/MOE), calendrier RDV exposé sur ContactRdvModal, publications sociales (LinkedIn/TikTok/Instagram), leads progressifs avec quota mensuel. **Complet en production.** |
| [fiches-drill-down.md](fiches-drill-down.md) | ⭐ **Phase 21 (18/05)** — Graph navigable BRH : 3 fiches drill-down (`adresse`, `entreprise`, `personne`) + page recherche `/recherche` dédiée. Remplace le modal slide-in V2 qui écrasait la navigation. 1 hop direct + sections dépliables lazy. RGPD-aware matrice 4 profils. Sprint 1+2 livrés (commits `fdcebd5`, `f36fa3f`). |
| [foncier-pro-blueprint.md](foncier-pro-blueprint.md) | 🏗️ **Phase 19 Blueprint (06/05)** — Architecture Foncier Pro agence `/agence/foncier/*` : 12 features Foncier Facile Plus + différenciateurs IA (résumé PLUi Claude + Vision IA toiture). 8 nouvelles tables, 4 EFs, 6 sprints A-F (~42-55j). Bretagne V1, all-free data publique. |
| [foncier-pro-status.md](foncier-pro-status.md) | 🟡 **Phase 19 Status (06/05)** — Sprint A en cours (foundation cadastre IGN + carte + favoris). 5 sprints suivants : SCI enrichi, DVF archive + sociodémo, PLU+Vision IA, BODACC+permis, UX intégrée. |
| [reseau-social-blueprint.md](reseau-social-blueprint.md) | 📐 **Phase 18 Blueprint (06/05)** — Architecture cible couche réseau social pro `/reseau` transverse 4 personae (agences/artisans/architectes/apporteurs) + 8 décisions stratégiques + 11 tables nouvelles + algo feed V1 + bridge AUTAF API (pas migration) + emprunts AUTAF (5/8 composantes ENGINE_V2). 🟡 **PAUSE** (à retravailler). |
