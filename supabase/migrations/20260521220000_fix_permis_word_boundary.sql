-- 2026-05-21 — Audit qualité — Fix faux positifs match permis
--
-- BUG DÉTECTÉ (audit Philippe 21/05) :
--   Le RPC brh_client_foncier_at_address (v Phase 6D) matche les permis
--   via `LIKE '%' || voie_norm || '%'` SANS word boundary.
--   Conséquences observées :
--     - voie "nc" matche "0057 RUE DES AJONCS"
--     - voie "armor" matche "LOTISSEMENT ARMORIQUE"
--   Pattern : voie courte contient des sous-chaînes communes → matches indus.
--
-- CORRECTIFS APPLIQUÉS :
--   1. length(voie_norm) >= 5 : élimine "nc", "ea", "x", etc.
--   2. Liste de sentinelles à exclure (artéfacts données manquantes)
--   3. Regex `~ '\mvoie\M'` au lieu de LIKE — \m et \M sont les word boundaries
--      Postgres POSIX. "nc" ne matchera plus "AJONCS" car "AJONCS" ≠ mot "nc".
--      Garde la perf : code_postal indexé filtre d'abord, puis regex sur le subset.
...
BEGIN
  ...
  -- Garde-fou matching : voie >= 5 chars + non-sentinelle
  v_voie_safe := v_client.voie_norm IS NOT NULL
              AND length(v_client.voie_norm) >= 5
              AND v_client.voie_norm NOT IN (
                'inconnu', 'non communique', 'xxx', 'fff', 'eas', 'hdos', 'boro', 'meot'
              );
  ...
  WITH
  dpe AS (
    -- Inchangé Phase 6D : match strict cp+num+voie (0% faux positif validé audit)
    SELECT jsonb_agg(
      jsonb_build_object(
        'dpe_id', d.id,
  ...
  dvf AS (
    -- Inchangé : match voie= (égalité stricte sur voie normalisée, pas LIKE)
    SELECT jsonb_agg(
      jsonb_build_object(
        'id', dvf.id,
  ...
  -- FIX faux positifs permis : word boundary regex au lieu de LIKE
  permis AS (
    SELECT jsonb_agg(
      jsonb_build_object(
...
COMMENT ON FUNCTION public.brh_client_foncier_at_address(uuid) IS
  'Audit qualité 21/05 — Word boundary regex sur match permis (élimine faux positifs LIKE %nc% → AJONCS). length(voie) >= 5 + sentinelles exclues.';
