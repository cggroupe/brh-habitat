-- 2026-05-18 — Ajoute `adresse_ban` au retour RPC + nouveau filtre `p_filter_particulier`.
--
-- Bug retour Philippe 18/05 fin :
--   1. La colonne `adresse` est tronquée ("Louis Pasteur", "des Brebis") alors
--      que `adresse_ban` (BAN-normalisée) contient le complet ("12 Rue Louis
--      Pasteur 29770 Audierne"). 99,97 % des leads ont une adresse_ban.
--   2. Pas de filtre "détenu par particulier" — pour voir les ~10k clients
--      BRH enrichis qui sont propriétaires particuliers (pas SCI).
--
-- Fix : ajout colonne retour `adresse_ban` + param `p_filter_particulier boolean`.
-- Reste : signature inchangée pour les autres params.
