...
WITH scored AS (
  SELECT
    ...
    -- NEW r21: BASIAS lourd (commune > 50 sites pollués potentiels = friche industrielle)
    CASE WHEN c.basias_count > 50 THEN -3 ELSE 0 END AS r_basias_lourd,
    -- NEW r22: SRU carencée = pression construction logement social = marché tendu
  ...
  score_v2_breakdown = jsonb_build_object(
    'rules', (
      SELECT jsonb_agg(r ORDER BY (r->>'points')::int DESC) FROM (
        ...
        UNION ALL SELECT jsonb_build_object('rule','abf_lourd','points',s.r_abf_lourd,'merimee_n',s.merimee_count,'abf_ac1',s.abf_ac1_count) WHERE s.r_abf_lourd < 0
        UNION ALL SELECT jsonb_build_object('rule','population_growth','points',s.r_pop_growth,'evol',s.evolution_pop_16_22) WHERE s.r_pop_growth > 0
        ...
        UNION ALL SELECT jsonb_build_object('rule','basias_lourd','points',s.r_basias_lourd,'sites',s.basias_count) WHERE s.r_basias_lourd < 0
        UNION ALL SELECT jsonb_build_object('rule','sru_carencee','points',s.r_sru_carencee) WHERE s.r_sru_carencee > 0
      ) sub
    ),
