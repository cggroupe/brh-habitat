...
interface ChiffrageAIChatProps {
...
export default function ChiffrageAIChat({
  partnerType,
  userId,
  userName,
  companyId,
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    try {
      ...
      if (chiffrage?.lignes && chiffrage.total_ttc) {
        ...
        if (userId) {
          saveChiffrage(chiffrageComplete, userId, companyId).catch((err) => logError('ChiffrageAIChat:saveChiffrage', err))
        }
      }
        ...
        <div className="bg-gradient-to-r from-primary-dark to-primary px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-body text-xs text-green-200">{subtitle}</p>
            </div>
          </div>
