...
interface ReseauPageProps {
  partnerType: 'pro' | 'particulier'
  subtitle: string
  recruitLinks: (userId: string) => RecruitLink[]
  formatCommission: (c: { commission_amount: number; source_type?: string }) => string
  formatTotalCommission: (stats: { total_commission_earned: number }) => string
...
export default function ReseauPage({
  ...
  const links = recruitLinks(user?.id ?? '')
  ...
  return (
    <div className="p-8 lg:p-10">
      ...
      {stats && stats.total_recruits > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
          {[
            ...
            { value: formatTotalCommission(stats), label: 'Commission gagnee', color: 'text-amber-600' },
      ...
      <div className={`grid grid-cols-1 ${links.length > 1 ? 'md:grid-cols-2' : ''} gap-4 mb-6`}>
        {links.map((link, idx) => (
          <div key={idx} className="bg-white rounded-2xl p-5 shadow-[0_8px_30px_rgba(27,28,28,0.04)]">
            <p className="text-[10px] uppercase tracking-widest font-bold text-text-light mb-3">{link.label}</p>
            <div className="flex gap-2">
              ...
              >
                {copiedIdx === idx ? <Check size={14} className="text-primary" /> : <Copy size={14} />}
              </button>
              <a
                href={getWhatsAppShareUrl(link.shareText, link.url)}
                target="_blank"
                  ...
                  <tbody>
                    {commissions.map((c, idx) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 font-display text-sm font-bold text-primary text-right">
                          +{formatCommission(c)}
                        </td>
