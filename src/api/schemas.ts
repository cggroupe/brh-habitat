...
export const brhProspectRowSchema = z.object({
  ...
  urgency: z.enum(['immediate', '3mois', '6mois', 'plus']).nullable(),
  status: z.enum(['nouveau', 'etude', 'devis_envoye', 'signe', 'termine', 'perdu']),
...
export const companyMemberWithProfileSchema = z.object({
  ...
  company_id: z.string(),
  profile_id: z.string(),
  member_role: z.enum(['owner', 'member']),
  joined_at: z.string(),
  profile: z.object({
    id: z.string(),
    email: z.string(),
...
export const brhSocialPostRowSchema = z.object({
  ...
  company_id: z.string().nullable(),
  platform: z.enum(['facebook', 'instagram', 'linkedin', 'tiktok', 'google_business']),
  post_type: z.enum(['post', 'video', 'article', 'review']),
  post_url: z.string(),
  ...
  status: z.enum(['en_attente', 'en_cours_verification', 'validee', 'refusee', 'expiree']),
...
export const prospectInsertSchema = z.object({
  ...
  estimated_budget: z.number().int().nonnegative().nullable().optional(),
  urgency: z.enum(['immediate', '3mois', '6mois', 'plus']).nullable().optional(),
  notes: optionalString,
})
