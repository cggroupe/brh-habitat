...
export default function PartPoints() {
  ...
  return (
    <div className="p-8 lg:p-10">
      {/* Header */}
      <div className="mb-8">
        <p className="text-[10px] uppercase tracking-widest font-bold text-text-light mb-1">Fidélité</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        <div className="bg-white rounded-2xl p-6 shadow-[0_8px_30px_rgba(27,28,28,0.04)]">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-[10px] uppercase tracking-widest font-bold text-text-light">Solde actuel</p>
          </div>
          <p className="font-display text-4xl font-bold text-primary">{balance.toLocaleString('fr-FR')}</p>
          <p className="text-sm text-text-light mt-1">points disponibles</p>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-[0_8px_30px_rgba(27,28,28,0.04)]">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-[10px] uppercase tracking-widest font-bold text-text-light">Total gagné</p>
          </div>
          <p className="font-display text-4xl font-bold text-text-primary">{totalEarned.toLocaleString('fr-FR')}</p>
          <p className="text-sm text-text-light mt-1">points cumulés depuis le début</p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-2xl p-14 shadow-[0_8px_30px_rgba(27,28,28,0.04)] text-center">
          <div className="w-16 h-16 bg-background rounded-2xl flex items-center justify-center mx-auto mb-5">
            <History size={28} className="text-text-light" />
          </div>
              ...
              return (
                <div key={tx.id} className="bg-white rounded-2xl p-5 shadow-[0_8px_30px_rgba(27,28,28,0.04)] flex items-center gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    {tx.description && (
                      <p className="text-sm text-text-secondary truncate">{tx.description}</p>
                    )}
                    <p className="text-xs text-text-light mt-1">{formatDate(tx.created_at)}</p>
                  </div>
                  <span className={`font-display text-lg font-bold shrink-0 ${isPositive ? 'text-primary' : 'text-red-500'}`}>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-background">
                  ...
                  <th className="text-left text-[10px] uppercase tracking-widest font-bold text-text-light px-6 py-4">Type</th>
                  <th className="text-left text-[10px] uppercase tracking-widest font-bold text-text-light px-6 py-4">Description</th>
                  <th className="text-right text-[10px] uppercase tracking-widest font-bold text-text-light px-6 py-4">Points</th>
                </tr>
              </thead>
              <tbody>
                {transactions.map(tx => {
                  ...
                  return (
                    <tr key={tx.id} className="hover:bg-background/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-text-secondary">
                        {tx.description ?? <span className="text-text-light/40">—</span>}
                      </td>
                      <td className={`px-6 py-4 font-display text-base font-bold text-right ${isPositive ? 'text-primary' : 'text-red-500'}`}>
                        {isPositive ? '+' : ''}{tx.points.toLocaleString('fr-FR')} pts
                      </td>
                    </tr>
                  )
